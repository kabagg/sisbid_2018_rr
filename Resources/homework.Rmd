...
Set up a new empty repo on GitHub.
Set up a new Project in RStudio, linking it to 
your GitHub repo. 

What files are in your project to begin with?
What does the git pane show?
...
For a recent project you've worked on, write
a README.Rmd file, setting the output type 
to github\_document. What was the project 
about? What data were you working with?
Are there relevant papers that have emerged
that should be linked to?

Knit the README to produce README.md. Add and commit
both versions of the README, and push them to GitHub.
Does the README look useful? What other info would 
be useful?

Set up the folder structure you prefer. 
Generate a small dataset for plotting, e.g.
...
Save the data in a csv file in your preferred folder.
Add a link to the csv file to your README. Add, commit, 
push. 

Write a script to load and plot the data, adding some
description of what you see. Use here to tell your 
script where to find the data. Add, commit, push. 

Write either a make\_all.R script or a Makefile
to run your first script above, leaving it general
enough that adding another script would be trivial. 
Use here to specify where the outputs should go. 
Add, commit, push. 

Edit the README to include links to your script
and a brief description of your workflow.
Add, commit, push. 

Is this complete?

Set up a second new RStudio project, link it to 
the GitHub repo above, and pull. When you run 
the scripts, do you get the same results?

Suggest something you might do that would cause
the results to be divergent. 

# Branching and Merging with Git/GitHub (Day 2, Session 2)

On your machine, create a new development branch
to add a second analysis script (e.g., producing
color versions of the plot above); don't link this 
one to GitHub. Draft and compile your script.
Add and commit. 

Look at the Git History popup. Do you see how 
your development branch differs from master?
Switch back and forth between the history for 
master and development to get a feel for the
changes. Look for the additions (green highlighting)
and deletions (reddish highlighting) in altered 
files (this is what git diff is doing).

Merge your development branch back into master.
What do the history displays look like now?
Prune this development branch, since you're done
with it. 

Set up another local (non GitHub) 
development branch. Change something
in the development branch, and then change the same
thing in the master branch in an incompatible way. 
Try to merge the development branch back into master.
Resolve the merge conflict, and add and commit the
final file with your revisions. What does the 
history look like now? Prune the branch when
you're done with it. 

Now add a development branch with a corresponding
tracking branch for GitHub. Make changes to the 
development branch, and Add, Commit, and Push. 
Set up a pull request, adding a few comments. 
Accept (or reject) the pull request. Use pulls to 
update your local versions of the repo. When you're
done with the development branch, prune it and the 
